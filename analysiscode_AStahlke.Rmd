...
fum<-with(gen_env,betadisper(commax,Site))#looks at multivariate dispersion (variance) among SITES
...
# points(soilz.mds,display="sites",col=gen_env$Site, choices=c(1,2),cex=1.5,lwd=1.5)
# text(soilz.mds,display = "sites",labels = gen_env$Site)
...
```{r}
# scores(soilz.mds)
soil.nut1<-soilz.mds$points[,"MDS1"]#soil variables for multimodel comparison
...
```{r}
cl_global_glm <- glm(Clone ~ Sand+grass+bare+soil.nut1+soil.nut2,
                    data=gen_env,family="binomial",
                    na.action = "na.fail",weights = rep(15,dim(gen_env)[1]))
summary(cl_global_glm)

cl_global_site <- glmer(Clone ~ Sand+grass+bare+soil.nut1+soil.nut2 
                     +(1|gen_env$Site),
                     data=gen_env, family="binomial", 
                     na.action = "na.fail",weights = rep(15,dim(gen_env)[1]))
...
cl_global_glm_pruned<-glm(Clone ~ 
                            Sand+grass+bare+soil.nut1_pruned+soil.nut2_pruned,
           data=gen_env_pruned, family="binomial",
           na.action="na.fail",
           weights = rep(12,dim(gen_env_pruned)[1]))
...
```{r}
stMod<-glmer(Clone ~ 
             soil.nut1+soil.nut2+grass+bare+(1|gen_env$Site),
             data=gen_env,family="binomial",na.action = "na.fail")
summary(stMod)

# stMod_cl_global1<-glm(Clone~Sand+grass+bare+soil.nut1,data=gen_env,family="binomial",na.action = "na.fail",weights = rep(15,dim(gen_env)[1]))
# summary(stMod_cl_global1)
...
```{r}
summary(glm(Clone~soil.nut1+soil.nut2,data=gen_env,family="binomial", weights = rep(15,dim(gen_env)[1])))

summary(glmer(Clone~soil.nut1+soil.nut2+(1|gen_env$Site),data=gen_env,family="binomial"))

summary(glmer(Clone~Sand+grass+bare+soil.nut1+soil.nut2+(1|gen_env$Site),data=gen_env,family="binomial"))

```

```{r}
stMod<-glmer(Clone~Sand+grass+bare+soil.nut1+soil.nut2+(1|gen_env$Site),data=gen_env,family="binomial",na.action = "na.fail")
summary(stMod)
```
...
# stMod<-glmer(Clone~grass+bare+Zn+Fe+OM+Mn+Na+(1|gen_env$Site),data=gen_env,family="binomial",na.action = "na.fail")
```

```{r}
summary(glmer(Clone~grass+bare+Fe+Mn+(1|gen_env$Site),data=gen_env,family="binomial",na.action = "na.fail"))

```

```{r}
summary(stMod<-glm(Clone~grass+bare+Zn+Fe+OM+Mn+Na,data=gen_env,family="binomial",na.action = "na.fail"))
```

```{r}
stMod <- glmer(Clone~grass+bare+Fe+Mn+(1|gen_env$Site),data=gen_env,family="binomial",na.action = "na.fail")
...
```{r}
summary(glmer(Clone~soil.nut1+(1|gen_env$Site),data=gen_env,family="binomial"))
```

#####stem density (1-inf)
```{r}
summary(lmer(stem~Sand+grass+bare+soil.nut1+soil.nut2+(1|gen_env$Site),data=gen_env))
stMod<-lm(stem~Sand+grass+bare+soil.nut1+soil.nut2,data=gen_env,na.action = "na.fail")
summary(stMod)
...
#### for some reason can't get this chunk to work
...
####CAnonical Analysis of Principal Coordinates (as in Andreson & Willis 2003, Biodiversity R package)
```{r}


# data(env_fact)
Ordination.model1 <- CAPdiscrim(env_fact~Site,
                                data=gen_env,dist="gower", axes=2, m=0, add=FALSE,
                                permutations = 100)
Ordination.model1
...
plot(sitz)
plot(sitz.text)
plot2 <- ordiplot(Ordination.model1, type="none",choices = c(1,3))
...
Ordination.model1 <- CAPdiscrim(soilz~factClone,data=gen_env,
                                dist="gower", axes=3, m=0, add=FALSE)
Ordination.model1
```
...
```{r}
### something funky going on here too
# plot2 <- ordiplot(Ordination.model1, type="none",choices = c(1,3))
# ordisymbol(plot2, gen_env,"Site", legend = T)
```
