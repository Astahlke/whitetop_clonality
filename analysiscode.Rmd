...
Global model includes Sand, grass, bare, soil, poca nutrients 1, pcoa nutrients 2
glmer allows inclusions of site as a random factor. 
...
Inclusion of site as random factor impoves AIC. 
...
cbind(gen_env[influential, ],soil.nut1.pcoa[influential],soil.nut2.pcoa[influential])
...
cbind(gen_env_pruned[influential,],
      soil.nut1.pcoa_pruned[influential],
      soil.nut2.pcoa_pruned[influential])
```
...
soil.nut1.pcoa_pruned4 <-res$vectors[-drop4,"Axis.1"]
soil.nut2.pcoa_pruned4 <-res$vectors[-drop4,"Axis.2"]


cl_global_glm_pruned4<-glm(Clone ~
                            Sand+grass+bare+
                            soil.nut1.pcoa_pruned4 + soil.nut2.pcoa_pruned4,
...
cbind(gen_env_pruned4[influential,],
      soil.nut1.pcoa_pruned4[influential],
      soil.nut1.pcoa_pruned4[influential])
...
```{r rescale_surfacevars}
max(soil.nut1.pcoa_pruned4) - min(soil.nut1.pcoa_pruned4)
max(soil.nut2.pcoa_pruned4) - min(soil.nut2.pcoa_pruned4)

max(soil.nut1.pcoa_pruned) - min(soil.nut1.pcoa_pruned)
...
cl_global_glmer_pruned_rescaled <-glmer(Clone ~ 
           I(Sand/100) + I(grass/100) +I(bare/100)+
             soil.nut1.pcoa_pruned +
             soil.nut2.pcoa_pruned                           
           +(1|gen_env_pruned$Site),
           data=gen_env_pruned, 
           family="binomial", 
           na.action = "na.fail",
...
stem_global_lmer_pruned_rescaled4 <-lmer(stem ~ 
           I(Sand/100) + I(grass/100) +I(bare/100)+
             soil.nut1.pcoa_pruned4 +
             soil.nut2.pcoa_pruned4                           
           +(1|gen_env_pruned4$Site),
           data=gen_env_pruned4, 
           na.action = "na.fail")
summary(stem_global_lmer_pruned_rescaled4)
```
