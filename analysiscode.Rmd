...
env_fact.D <- vegdist(env_fact, "gower")
env_fact.res <- ape::pcoa(env_fact.D)
env_fact.factors<-envfit(env_fact.D, env_fact, perm=999)

biplot(env_fact.res, rn=gen_env$Site,legend=TRUE,cex=2,lwd=2,choices = c(1,2))
plot(env_fact.factors, p.max=.05)

#soil variables for multimodel comparison
env_fact.pcoa1<-env_fact.res$vectors[,"Axis.1"]
env_fact.pcoa2<-env_fact.res$vectors[,"Axis.2"]

barplot(env_fact.res$values$Relative_eig)
cumsum(env_fact.res$values$Relative_eig)
```
...
```{r global_glm_glmer}
cl_global_forb_glm <- glm(Clone ~ Sand+grass+forb+
...
cl_global_forb_site <- glmer(Clone ~                                    Sand+grass+forb+
                  soil.nut.pcoa1 +
